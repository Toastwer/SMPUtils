...
public class RankChatColorCommand extends RankArgumentHandler {
    public RankChatColorCommand(SMPUtils main, RankManager rankManager) {
    ...
    public void handle(CommandSender sender, String label, String[] args) {
        if (args.length < 2) {
            main.sendMessage(sender, "§cThe command should be in the format §4/rank chatolor <rank> §o[new color]");
            return;
        }
        ...
        if (args.length < 3) {
            for (RankManager.Rank rank : rankManager.getAllRanks())
                if (rank.name.equals(args[1]))
                    main.sendMessage(sender, "§fThe rank §6" + args[1] + "§f has the chat color '" + rank.chatColor + "some example text§f'");

            return;
        }

        rankManager.setChatColor(args[1], args[2]);
        main.sendMessage(sender, "§aThe rank §o" + args[1] + "§a now has the chat color '" + args[2].replace('&', '§') + "some example text§a'");
    }
